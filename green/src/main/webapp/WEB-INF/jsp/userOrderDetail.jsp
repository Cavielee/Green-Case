		...
		<div class="container">
			...
			<div class="row clearfix">
				<div class="col-xs-12 recycle">
					<c:if test="${orderDetail.order.status == 0 }">
						<a href="cancelOrder?order_id=${orderDetail.order.order_id }">取消订单</a>
					</c:if>
					<c:if test="${orderDetail.order.status == 1 }">
						<a href="javascript:history.go(-1)">确认</a>
					</c:if>
				</div>
			</div>
