...
<link rel="stylesheet" type="text/css" href="css/untreatedOrders.css">
...
<body>
	<div class="bg">
		<div class="container" id="order_item">
			<div class="row clearfix" id="topbar">
				...
				<div class="col-xs-6">
					<h3 class="title">未处理订单</h3>
				</div>
			</div>
			<c:forEach items="${page.list }" var="order">
				<a onClick="orderDetail(this)">
					<div class="row clearfix order_item">
						<div class="col-xs-12">
							<ul>
								<li name="username">用户名
									<p name="username">${order.username }</p> <i
									class="fa fa-angle-right"></i>
								</li>
								<li name="order_id">${order.order_id }</li>
								<li>下单时间
								...
								<li>订单状态 <c:if test="${order.status== false }">
			...
			<form action="untreatedOrderDetail" method="post">
		...
		<div id="page">
			<ul>
				...
				<br />
				<a href="untreatedOrders?pageNum=1"><li class="first_page">首页</li></a>
				<c:choose>
					<%--只有一页 --%>
					...
					<c:when test="${page.pageNum ==1 && page.totalPage > 1}">
						<c:forEach begin="${page.start}" end="${page.end}" step="1"
							...
							<c:if test="${page.pageNum != i}">
								<a href="untreatedOrders?pageNum=${i }"><li class="page">${i}</li></a>
							</c:if>
						</c:forEach>
						<a href="untreatedOrders?pageNum=${page.pageNum+1 }"><li
				...
				<%--尾页 --%>
				<a href="untreatedOrders?pageNum=${page.totalPage }"><li class="last_page">尾页</li></a>
			</ul>
		</div>
	...
	<script src="js/untreatedOrders.js"></script>
</body>
</html>