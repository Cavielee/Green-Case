	...
	<div class="bg">
		...
		<div id="page">
			<ul>
				...
				<br />
				<a href="goodsManage?pageNum=1"><li class="first_page">首页</li></a>
				<c:choose>
					<%--只有一页 --%>
					...
					<c:when test="${page.pageNum ==1 && page.totalPage > 1}">
						<c:forEach begin="${page.start}" end="${page.end}" step="1"
							...
							<c:if test="${page.pageNum != i}">
								<a href="goodsManage?pageNum=${i }"><li class="page">${i}</li></a>
							</c:if>
						</c:forEach>
						<a href="goodsManage?pageNum=${page.pageNum+1 }"><li
				...
				<%--尾页 --%>
				<a href="goodsManage?pageNum=${page.totalPage }"><li class="last_page">尾页</li></a>
			</ul>
		</div>
