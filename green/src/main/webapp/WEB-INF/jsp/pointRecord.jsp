...
<link rel="stylesheet" type="text/css" href="css/pointRecord.css">
...
<body>
	<div class="bg">
		<div class="container">
			...
			<div class="row clearfix" id="pointRecord">
				<c:forEach items="${page.list }" var="pointRecord">
					<div class="col-xs-12 pointRecord">
						<div>
							<i class="circle"></i>
							<p>${pointRecord.created }</p>
							，${pointRecord.description }
							<p class="point">
								<c:if test="${pointRecord.point > 0 }">+</c:if>
								${pointRecord.point }
							</p>
							积分
						</div>
		...
		<div id="page">
			<ul>
				...
				<br />
				<a href="pointRecord?pageNum=1"><li class="first_page">首页</li></a>
				<c:choose>
					<%--只有一页 --%>
					...
					<c:when test="${page.pageNum ==1 && page.totalPage > 1}">
						<c:forEach begin="${page.start}" end="${page.end}" step="1"
							...
							<c:if test="${page.pageNum != i}">
								<a href="pointRecord?pageNum=${i }"><li class="page">${i}</li></a>
							</c:if>
						</c:forEach>
						<a href="pointRecord?pageNum=${page.pageNum+1 }"><li
				...
				<%--尾页 --%>
				<a href="pointRecord?pageNum=${page.totalPage }"><li
